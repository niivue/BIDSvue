...
In-app assistant running Claude Code / Codex / Gemini CLIs or direct OpenAI-compatible endpoints against the dataset, via a BIDSvue MCP server (CLIs) or an app-owned tool-call loop (direct). Every mutation routes through `MutationLease` + `OperationContext` + `operations.log` with a human Approve/Reject gate. **AI now runs on Windows (experimental)**: the control bridge is cross-platform (Unix-domain socket on Unix, named pipe on Windows secured by a per-user-SID DACL), the Windows child is assigned to a `KILL_ON_JOB_CLOSE` Job Object, and the pipe relay/client have real Windows tests. It is still not called shipped until a clean-VM end-to-end Windows CLI session is smoked. (The broader Windows workspace test gate is now path-portable — the `process`/`trust` POSIX-path fixtures route through the `testpath::abs` helper, so `cargo test --workspace` passes on Windows too.) Pending:

1. **`ReadPolicy` struct** — consolidate the per-session 64 MiB egress cap, JSON-RPC frame cap, session-config size cap onto `ServerContext`; charge before serialization.
2. **`AiChildSession` RAII guard** — one struct owning session dir + cancellation registration + child + stream tasks (closes the exception-safe-cleanup + channel-send-leak + register-error paths).
3. **Hardened-runtime smoke** — full notarized DMG → Finder-launch → spawn CLI → CLI spawns `bidsvue --mcp-server` → assert no FD-plumbing deadlock.
4. **HistoryDialog UI** — surface AI-session groups + revert + intervening-conflict refusal.
5. **Windows AI (RELEASE CANDIDATE)** - every known code/security concern raised through round 7 is closed and machine-verified (`cargo test ai::` green, `cargo test --workspace` green on Windows); only a manual clean-VM smoke remains. The Windows bridge uses a per-user-SID named pipe with an explicit `O:{sid}` owner + `OICI`-inheritable DACL, fail-fast first-instance creation, pipe-owner-SID verification before bearer send, 20s/130s client read timeouts + a 30s server request-line deadline, bounded-backoff pipe connect/re-create recovery, `JoinSet`-tracked handlers aborted on teardown, retrying telemetry (advance-on-success), direct `.cmd`/`.bat` shim launch (no `cmd /C`; BatBadBut regression pinned), restricted PATHEXT discovery, absolute `%SystemRoot%\System32` PowerShell/taskkill launches, fail-closed session/workdir/config ACL hardening + no-clobber credential writes, fail-closed CLI resolution (no bare-`argv[0]` fallback), cross-platform post-open fd TOCTOU verification (macOS `F_GETPATH` / Linux `/proc/self/fd` / Windows `GetFinalPathNameByHandle`), required write-argument validation at the Rust authority layer, `.git/annex` blocked on the content-write path, a non-string `path` rejected, and a `KILL_ON_JOB_CLOSE` Job Object reaping descendants on cancel + normal exit. Tests cover the relay round-trip, injection, PATHEXT, no-clobber, request-line timeout, retry predicate, fail-closed launch, fd-scope verification, and required-arg rejection. Two low-severity residuals are documented and accepted (Job-Object assign race; control-pipe PERSISTENT re-create failure — transient failures self-heal via bounded backoff — see LIMITATIONS.md). **The single remaining gate to drop "experimental" and merge: a manual clean-VM Claude/Codex/Gemini full-session smoke with a real approved write.** The earlier Windows `cargo test --workspace` path-assumption failures are fixed (the `process`/`trust` POSIX-path fixtures route through the `testpath::abs` helper).
