...
| Viewer | `@niivue/niivue` consumed via a `file:` dep against the sibling in-tree rc.8 build at `../mono/packages/niivue` (M-PHY0); re-pin to npm rc.8+ before release. Release script blocks notarisation while any `file:`/`link:`/relative dep spec or `bunfig.toml` linker workaround is present. |
...
| `NiiVue` | npm package | Renderer | All | Pure JS + WebGL2. |
...
- **dcm2niix-reproin** runs BIDSvue's port of the upstream `dcm2niix/tools/reproinx.py` post-pass (latest sync: 2026-06-20, commit `af7559a`). It collapses non-reproin double-root output, rescues `Unknown/*` from `BidsGuess` + provenance TSV, cleans bidsguess artifacts, renames duplicate output stems, backfills sessions, rescues physio from `derivatives/scanner/`, writes scans/events/root scaffolding, and optionally drops `derivatives/scanner/`. Load-bearing gotchas: Unknown physio rescue runs in a `finally` after BOLD rescue; family moves go through atomic `moveStemFiles`; session inference is gated to `BIDS_DATATYPES` so `code/` / `stimuli/` / `sourcedata/` cannot steer it; physio rescue requires both `.tsv.gz` and `.json`, and a half-pair warning preserves `derivatives/scanner/` instead of deleting the remaining companion.
...
- **E2E** (WebdriverIO + `tauri-driver`): happy-path open-and-validate against `tests/fixtures/tiny-bids/`. Renderer honors `BIDSVUE_TEST_OPEN_DATASET=<abs-path>` to bypass the launch screen.
...
- **Viewer:** [`@niivue/niivue`](https://github.com/niivue/niivue) — consumed via a `file:` dep against `../mono/packages/niivue` (the sibling in-tree rc.8 build, M-PHY0) so the renderer can call `loadSignals(...)` / `removeAllSignals()` / `setSignal(...)` for physio overlays. Default export is `NiiVueGPU`; `destroy()` exists. **Re-pin to npm rc.8+ before any notarised DMG** — `scripts/macos-release.sh` blocks release while any dep spec starts with `file:` / `link:` / `./` / `../` (across `dependencies`, `devDependencies`, `optionalDependencies`, `peerDependencies`, AND `overrides`) OR while `bunfig.toml` keeps the `linker = "hoisted"` dev-loop workaround.
...
- **CI:** GitHub Actions. The notarized macOS release path remains local; the cross-platform bundle workflow builds unsigned Linux x86_64 and Windows x86_64 artifacts from committed sidecars. Static checks (lint / typecheck / Bun tests / cargo fmt + test) and scanner bench continue to run on Linux.
