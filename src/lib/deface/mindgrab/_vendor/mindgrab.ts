...
//
// Excluded from biome via the `files.ignore` entry in `biome.json` — the
// upstream `biome-ignore-all lint:` directive is biome 2.x syntax and
// doesn't parse on biome 1.9.4.
//
// Vendored from @niivue/nv-ext-brain2print/src/models/mindgrab.ts
// (the package is an unpublished technology demo; we copy the model
// rather than depend on it). Upstream is at github.com/niivue/niivue's
// mono workspace `packages/nv-ext-brain2print/`. Refresh by re-copying
// when the upstream model is regenerated by tinygrad.
