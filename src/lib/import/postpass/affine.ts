...
// from reproinx.py. We resolve the "best" affine the same way nibabel
// does: sform when sform_code > 0, fall back to qform when
// qform_code > 0, otherwise return null. The post-pass treats a null
// here as "skip fmap-matching for this target".

import type { NiftiHeader } from './niftiHeader'
...
/**
 * NIfTI-1 quaternion (qform) → 4x4 affine matrix. Mirrors the C
 * reference implementation in nifti1.h's `nifti_quatern_to_mat44`.
 *
 *   - qfac = pixdim[0]; restricted to {-1, +1}; defaults to +1 otherwise.
 *   - a = sqrt(max(1 - b^2 - c^2 - d^2, 0))
 *   - rotation matrix R is built from (a, b, c, d) in standard form.
 *   - voxel sizes sx, sy, sz scale R's columns.
 *   - qoffset_x/y/z is the translation column.
 *
 * Returns a 4x4 readonly tuple; the bottom row is always [0, 0, 0, 1].
 */
 ...
 * `get_best_affine`. Returns null when neither sform nor qform is
 * usable; reproinx.py's `_imaging_volume` does the same.
 */
export function getBestAffine(
  header: NiftiHeader,
): { affine: Affine; shape3: Shape3 } | null {
  const shape3: Shape3 = [
    header.dim[1] ?? 0,
    header.dim[2] ?? 0,
    header.dim[3] ?? 0,
